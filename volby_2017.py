...
def data_obci(slovnik_obci: dict):
    ...
    for kod, link in slovnik_obci.items():
        ...
        for strana in soup_obec.find_all("td", {"class": "overflow_name"}):
          dict_obsahu[strana.text] = soup_obec.find("td", string=re.compile(strana.text[:20])).next_sibling.next_sibling.text
